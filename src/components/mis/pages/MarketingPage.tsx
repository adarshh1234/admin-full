...
import { ReferAndEarnTab, PromotionTab } from '../../pages/MarketingPage';

type MarketingSubTab = 'refer-earn' | 'promotion' | 'performance';

export default function MarketingPage() {
  const [activeSubTab, setActiveSubTab] = useState<MarketingSubTab>('refer-earn');
  ...
  return (
    <section id="tab-marketing" className="mis-tab-panel">
      {/* Sub-tab switcher */}
      <div className="mis-tab-nav" style={{ marginBottom: 20, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <button
          type="button"
          className={`mis-tab-btn${activeSubTab === 'refer-earn' ? ' mis-tab-btn-active' : ''}`}
          onClick={() => setActiveSubTab('refer-earn')}
        >
          <i className="fas fa-gift" /> Refer &amp; Earn
        </button>
        <button
          type="button"
          className={`mis-tab-btn${activeSubTab === 'promotion' ? ' mis-tab-btn-active' : ''}`}
          onClick={() => setActiveSubTab('promotion')}
        >
          <i className="fas fa-tags" /> Promotion
        </button>
        <button
          type="button"
          className={`mis-tab-btn${activeSubTab === 'performance' ? ' mis-tab-btn-active' : ''}`}
          onClick={() => setActiveSubTab('performance')}
        >
          <i className="fas fa-chart-bar" /> Digital Performance
        </button>
      </div>

      {activeSubTab === 'refer-earn' && <ReferAndEarnTab />}

      {activeSubTab === 'promotion' && <PromotionTab />}
